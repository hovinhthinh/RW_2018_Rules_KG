...
------------------------------------}\bigskip
  ...
  \uncover<3->{\gr{$\mi{livesIn(roger,bottmingen)}$\\}}
\end{column}
\begin{column}{.49\textwidth}
  ...
  \uncover<3->{\emph{Roger lives in Bottmingen}}
\end{column}
\end{columns}
\bigskip
...
%\begin{picture}(0.5,0.5)
    % \put(50,-136){
    \uncover<3->{\includegraphics[width=0.3\textwidth]{bottmingen}}
%}
   % \end{picture}
\end{center}
\end{frame}
...
\begin{beamerboxesrounded}[upper=uppercolred,lower=lowercolred,shadow=true]{\textbf{\alert{Important problems}} of KGs:}
 ...
 \item[\textbf{\alert{\circled{2}}}] \textbf{\alert{Incompleteness}}
 \end{itemize}
...
\begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{\textbf{\bl{In tutorial: }} Reasoning on top of KGs to address their incompleteness}
\bigskip

 \begin{itemize}
 \item[] \bl{Induction}: learning common-sense rules and \bl{completing} KGs
 \end{itemize}
\bigskip
...
\begin{frame}
\frametitle{Positive Logic Programs}


%%\normalsize

\begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{Def.: \textbf{Positive logic programs}}
A \bl{positive logic program}, $P$, is a finite set of
\bl{rules (clauses)} of the form
%
%
\begin{equation}
 \label{eq:clause}
 \bl{a \implied b_1,\ldots,b_m,}
\end{equation}
%
...
% \begin{example}
\bi
\item \textbf{Ground rule:} \emph{``If Franfurt
is a hub airport, and there is a link between Frankfurt and Saarbr{\"u}cken, then Saarbr{\"u}cken is a connected airport.''}

\vspace{-.2cm}
$$
\begin{array}{r@{}l}
\gre{\mathit{connected}(\mathit{srb})} \gre{\implied} & \gre{\mathit{hub\_airport}(\mathit{frankfurt}), \mathit{link}(\mathit{frankfurt},\mathit{srb})} \\
% connected(X) \implied & hub(Y), link(Y,X)
\end{array}
$$
...
\ms
\item \textbf{Non-ground rule:} \emph{``All airports with a link to a hub airport are connected.''}
\gre{$$
\begin{array}{r@{}l}
\mi{connected(X)} \implied & \mi{hub\_airport(Y), link(Y,X)}
\end{array}
$$}
...
%
\gre{$$\forall X,Y\; \mathit{hub\_airport}(Y) \wedge \mathit{link}(Y,X) \rightarrow \mathit{connected}(X).$$}
...
\textbf{Given:}
$$\gre{P_1 = \{ a \implied b.}\quad \gre{\ b \implied c.}\quad \gre{\ c \},}$$
...
\textbf{Given:}
$$\gre{P_2 = \{ a \implied b}.\quad \ \gre{b \implied a.}\quad \ \gre{c \},}$$
...
\begin{itemize}
                \item
For \gre{$P_1 = \{\; a \implied b.\quad \ b \implied c.\quad \ c \; \}$},
we have \gre{$LM(P_1) = \{ a,b, c\}$}.

\bs

\item For
\gre{$P_2 = \{\; a \implied b.\quad \ b \implied a.\quad \ c \;\}$},
