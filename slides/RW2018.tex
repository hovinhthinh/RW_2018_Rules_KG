...
\begin{beamerboxesrounded}[upper=uppercolgreen,lower=lowercolgreen,shadow=true]{\alt<2->{Example: non-ground rules}{Example: ground rules}}
\medskip
\small{
...
\bi
\item
Given a logic program $P$, the \bl{Herbrand universe} of $P$, \bl{$\HU(P)$}, is the set of all terms
which can be formed from constants and functions symbols in $P$ (resp.,
the vocabulary $\Phi$ of $P$,
% of $L$,
if explicitly known).
...
\item
The \bl{Herbrand base} of $P$, \bl{$\HB(P)$}, is the set of all ground
atoms which can be formed from predicates and terms $t\in \HU(P)$.
\pause

\s
\item
A \bl{(Herbrand) \bl{interpretation}} is a first-order interpretation \bl{$I =
(D,\cdot^I)$} of the vocabulary with domain \bl{$D=\HU(P)$} where each term $t \in \HU(P)$
is interpreted by itself, i.e., \bl{$t^I = t$}.

\pause

\s
\item
$I$ is identified with the set \bl{$\{\; p(t_1,\ldots,t_n) \in \HB(P) \mid
\tuple{t_1^I,\ldots,t_n^I}\in p^I\; \}$}.
\ei
\end{beamerboxesrounded}
\bs
\pause

Informally, a (Herbrand) interpretation can be seen as a set denoting
which ground atoms are true in a given scenario.

%\small{Named after logician Jacques Herbrand.}
...
\begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{Def.: \textbf{Herbrand models}}
\bs

An interpretation $I$ is a \bl{(Herbrand) model} of

\begin{itemize}
\item a ground (variable-free) \bl{clause} $C = a \implied b_1,\ldots,b_m$,  symbolically $I\models C$,
if either $\{b_1,\ldots,b_m\} \nsubseteq I$ or $a \in I$;
%  \hfill ($I\models C$)

\bs

 \item a clause $C$, symbolically $I\models C$,
...
\item a program $P$, symbolically $I\models P$,
...
\begin{itemize}
  \item  \gre{$I_1 = \emptyset$}  \visible<2->{ \red{\textbf{no}}}
  \item  \gre{$I_2 = \HB(P)$}  \visible<3->{ \gre{\textbf{yes}}}
  \item  \gre{$I_3 = \{ h(0,0), t(a,b,r), p(0,0,b)\}$} \visible<4->{ \red{\textbf{no}}}
\end{itemize}
...
\end{column}
\begin{column}{0.33\textwidth}
\begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{\begin{center}\bl{Induction}\end{center}}
\begin{center}\bigskip\bigskip
...
\begin{frame}\frametitle{History of ILP}
\begin{itemize}
...
\small{
 \textbf{Given:} 
 \begin{itemize}
 \item[\gre{$\circ$}] \gre{$T=\{\mi{parentOf(john,mary),male(john),}$\\$\phantom{T=\{}\mi{parentOf(david,steeve),male(david),}$\\$\phantom{T=\{}\mi{parentOf(kathy,ellen),female(kathy)}\}$}
\item[\gre{$\circ$}] \gre{$E^+=\{\mi{fatherOf(john,mary),fatherOf(david,steve)}\}$} 
\item[\gre{$\circ$}] \gre{$E^-=\{\mi{fatherOf(kathy,ellen),fatherOf(john,steve)}\}$} 
\item[\gre{$\circ$}] Target language: Horn rules
 \end{itemize}
\bigskip
\bigskip
...
\begin{itemize}
\item[\gre{$\circ$}] \gre{$\mi{Hyp:\;fatherOf(X,Y)\leftarrow parentOf(X,Y),male(X)}$}
\end{itemize}}
...
\smallskip
 ...
 \begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{Inductive Learning from Interpretations \cite{DBLP:journals/ngc/Muggleton91}}
 \smallskip
...
\begin{frame}\frametitle{Example}
 ...
 \begin{beamerboxesrounded}[upper=uppercolgreen,lower=lowercolgreen,shadow=true]{Inductive Learning from Interpretations \cite{DBLP:journals/ngc/Muggleton91}}
 \smallskip

 \textbf{Given:} 
 \begin{itemize}
\item[\gre{$\circ$}] \gre{$\mi{I=\{isMarriedTo(mirka,roger), livesIn(mirka,b),}$\\$\phantom{I=\{}\mi{livesIn(roger,b), bornIn(mirka,b)\}}$} 
\item[$\gre{\circ}$] \gre{$\mi{T=\{isMarriedTo(mirka,roger);\,bornIn(mirka,b);}$\\$\phantom{I=\{}\mi{livesIn(X,Y)\leftarrow bornIn(X,Y)}\}$}
\item[$\gre{\circ}$] Target rules: Horn rules
 \end{itemize}
...
\bigskip
 ...
 \begin{itemize}
\item \gre{$\mi{Hyp:livesIn(Y,Z)\leftarrow isMarriedTo(X,Y),livesIn(Y,Z)}$} 
\end{itemize}}
 \end{beamerboxesrounded}
\end{frame}
...
\begin{frame}\frametitle{Specificity of Knowledge Graph Setting}
\end{frame}
