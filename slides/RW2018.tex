...
\end{column}
\end{columns}
\uncover<3->{\includegraphics[width=.6\textwidth]{rogerInBot}}
%\begin{picture}(0.5,0.5)
    % \put(50,-136){
%}
   % \end{picture}
...
livesIn(mirka,b). \qquad isMarriedTo(mirka,roger).
\end{align*}
}
...
\item[\gre{$\bullet$}] \gre{The ground instances of the first rule are}
\ei
%
{\small\begin{align*}
livesIn(m,m) &\leftarrow  livesIn(m,m), isMarriedTo(m,m). & X=Y=Z=m\\
 &\ \ \ \ldots \\
livesIn(r,b) &\leftarrow  livesIn(r,r), livesIn(r,b). & X=Y=r, Z=b \\
 &\ \ \ \ldots \\
livesIn(b,b) &\leftarrow  livesIn(b,b), isMarriedTo(b,b). & X=Y=Z=b \\
 \end{align*}}
\end{beamerboxesrounded}
\end{frame}
...
\begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{Def.: \bl{\textbf{Herbrand models of a Horn program}}}
\bs

An interpretation $I$ is a \bl{(Herbrand) model} of (or satisfies)
...
\gre{Which of the following interpretations are models of $P$?}
\begin{itemize}
...
\begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{Def.: \bl{\textbf{Herbrand model, answer set}}}
\begin{itemize}
...
\small{$P = \left\{
      \begin{array}{l@{\qquad}}
         ...
         \phantom{\mbox{(3) }\mi{livesIn(\alt<3->{mat,ulm}{Y,Z})} \leftarrow }\naf\ \mi{researcher(\alt<3->{\mi{mat}}{Y})}; \only<4->{\\[0.35ex]\bl{\mbox{(4) }\mi{researcher(mat)}}}
...
\begin{center}\bigskip\bigskip\bigskip\bigskip
...
\gre{\emph{John and Mary live together. They must be a married.}}
\end{center}
\end{beamerboxesrounded}}
\end{column}
\begin{column}{0.3\textwidth}
...
\gre{\emph{\emph{John and his spouse Mary live together, Tom and his spouse Ann live together....$\Rightarrow$ All married people live together.}}}
\end{center}
...
\gre{\emph{John and his spouse Mary live together, Tom and his spouse Ann live together....$\Rightarrow$ All married people live together.}}
\end{center}
\end{beamerboxesrounded}}}
\end{column}
...
\item \bl{Syntactic restrictions} on the definition of $p$
 \end{itemize}
...
\item[\gre{$\bullet$}] \gre{Target language}: Horn rules
 \end{itemize}
\bigskip
\bigskip
...
\item[$\gre{\bullet}$] \gre{Target rules}: Horn rules
 \end{itemize}
...
\bigskip
 ...
 \begin{itemize}
\item[$\gre{\bullet}$] $\mi{\gre{Hyp}:livesIn(Y,Z)\leftarrow isMarriedTo(X,Y),livesIn(Y,Z)}$
\end{itemize}}
 \end{beamerboxesrounded}
\end{frame}
...
%\begin{itemize}
% \item $\geq 20$ years of ground-breaking research
%\end{itemize}
\begin{itemize}
\item \bl{Clause refinement} \cite{clauseref}
\begin{itemize}
\item Explore search space of clauses from simple to complex (general to
specific) or complex to simple (specific to general) to find a hypothesis that will cover all of the examples.
\end{itemize} 
\end{itemize}
\bs
...
\begin{frame}\frametitle{Common Techniques in ILP}
\begin{itemize}
\item \bl{Inverse entailment} \cite{}
\begin{itemize}
...
\begin{center}\includegraphics[width=0.7\textwidth]{hr_learn}
...
\begin{frame}\frametitle{\alt<3>{Inductive Learning from Interpretation}{\alt<2>{Example}{Declarative Programming}}}
\alt<2>{
 \begin{picture}(0.5,0.5)
...
\only<1>{\begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolblue,shadow=true]{\bl{Goal of ILP:}}Develop a correct (and complete) algorithm which efficiently computes hypotheses.. But world knowledge is complex, and this is not always possible due to several issues...
\end{beamerboxesrounded}}
\pause
...
\bigskip
\begin{center}
\includegraphics[width=0.2\textwidth]{man_bites_dog} 
\end{center}
\end{frame}
...
You decide on \bl{associative advertising}: show ads of
relevant products \bl{before} your users search for these
\bigskip

\begin{itemize}
\item \bl{Task:} Given a transaction database, find out products that are frequently bought together and form recommendation rules.
\end{itemize}
...
Apples and beer are bought together is 3 out of 8 cases.
\end{center}
\end{beamerboxesrounded}
\end{frame}
...
\begin{itemize}
\item \textbf{\bl{Propositionalization}}
\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Problem Statement}
 \begin{beamerboxesrounded}[upper=uppercolblue,lower=lowercolbl,shadow=true]{\textbf{\bl{Rule Learning Guided by External Sources}}}

\textbf{Given:} 
\begin{itemize}
\item Knowledge Graph $\mathcal{G}$
\item External Source  $\mathcal{P}$
\begin{itemize}
\item Probability function reflecting trustfulness of $\mathcal{G}$'s missing facts
\end{itemize}
...
\textbf{Find:} 
\begin{itemize}
\item Set of rules of the desired type, which agree with the external source
...
\uncover<2->{
\begin{picture}(0.5,0.5)
\put(292,45){\includegraphics[width=.17\textwidth]{external_source}}
\end{picture}
\begin{picture}(0.5,0.5)
\put(-10,70){\includegraphics[width=.25\textwidth]{kg}}
\end{picture}
\begin{picture}(0.5,0.5)
\put(120,-10){\includegraphics[width=.33\textwidth]{feedback}}
\end{picture}
\begin{picture}(0.5,0.5)
\put(135,130){\includegraphics[width=.22\textwidth]{rules}}
\end{picture}}
\end{frame}
%---------------------------------------------
...
\item[\bl{Q3}] \textbf{(Learn)} Can anything be learnt from the feedback provided?
...
\small{
						...
						\gr{General Solution:} Introduce the novel \bl{\textbf{Hybrid Rule Quality Function} \pmb{$\mu$}}:
						\begin{itemize}
							\pause
							\item Measure the \bl{\textbf{descriptive quality} \pmb{$\mu_1$}} of rule $r$ over $\mathcal{G}$:
							      \gr{\[\mu_1: (r,\cG) \mapsto \alpha \in  [0,1]\]}
							      $\Rightarrow$ any \textbf{\bl{classical quality measure}} which describes $\cG$ properly\\ (e.g. conf, PCA conf)
							\pause							      							
							\item Measure the \bl{\textbf{predictive quality} \pmb{$\mu_2$}} of $r$ based on the \bl{\textbf{external source  $\mathcal{P}$}}:
							      \gr{\[\mu_2: (r{,}\, \mathcal{P}) \mapsto  \alpha \,{\in}\, [0,1]\]}
%							      where $\mathcal{G}_r$ is a extension of $\mathcal{G}$ by applying $r$\\ %$\Rightarrow$ capture \textbf{\bl{information about missing facts}} in $\cG$ that are relevant for $r$
							...
							\item Combine the result as \bl{\textbf{the weighted sum}}:
							      \gr{\[\mu(r)= (1 - \lambda) \times \mu_1(r,\cG) + \lambda \times \mu_2(r,\mathcal{P})\]}
							where $\lambda$ is a weighting factor\\
  							\pause
							\alert{$\Rightarrow$ Need to define $\mu_2$}
...
\small{\begin{itemize}
\item[] Mimic \bl{``mine-interact-learn-repeat''} schema \scriptsize{\cite{interpat}}
\item[] \small{Establish \bl{``user-in-the-loop''} inspired interaction between the
rule mining algorithm and the embedding model}
...
\alt<2->{
\begin{picture}(0.5,0.5)
\put(290,55){\includegraphics[width=.35\textwidth]{kgembed}}
\end{picture}
}{
\begin{picture}(0.5,0.5)
\put(292,45){\includegraphics[width=.17\textwidth]{external_source}}
...
\begin{picture}(0.5,0.5)
\put(-7,75){\includegraphics[width=.25\textwidth]{kg}}
\end{picture}
\begin{picture}(0.5,0.5)
\put(125,0){\includegraphics[width=.33\textwidth]{feedback}}
\end{picture}
\begin{picture}(0.5,0.5)
\put(138,140){\includegraphics[width=.22\textwidth]{rules}}
\end{picture}
\end{frame}
...
\begin{frame}\frametitle{Q1 (Interact)}
Measure predictive quality \bl{$\mu_2$} of \bl{ $r: \mi{h(X,Y)\leftarrow B}$}, based on the embedding model\bigskip
...
\begin{center}
\bl{$\mi{\mu_2(r)}=\dfrac{1}{|predictions(r)|} \sum\limits_{{<}s\,p\,o{>}\in predictions(r)}\mi{rank({<}s\,p\,o{>})}$}
\end{center}
\end{itemize}
\bigskip
\pause
...
$\bl{\mu_2(r){=}}\dfrac{\bl{rank(}{<}\colorbox{lightbl}{Jack}\,\mathbf{\gr{livesIn}}\,\colorbox{lightgr}{NY}{>}\bl{)}{\bl{+}}\bl{rank(}{<}\colorbox{lightbl}{Mat}\,\mathbf{\gr{livesIn}}\,\colorbox{lightgr}{Berlin}{>}\bl{)}}{\bl{2}}$}}
\end{center}
% \bigskip
...
%\begin{center}
%\bl{$\mi{\mu_2(r)}=\dfrac{1}{|N|} \sum\limits_{s,h,o\in N}\dfrac{1}{\mi{rank(s,h,o)}}$}
%\end{center}
%\bigskip
%
...
\uncover<2->{- compute statistics: \bl{$\mu_1, \mu_2, \mu$}\\
...
- add \textbf{\bl{exception}} unary atom\\
- add \textbf{\bl{exception}} binary atom}
\end{itemize}
...
\begin{frame}\frametitle{System Overview}
 \begin{figure}[t]
\includegraphics[width=0.65\textwidth]{figures/mining.pdf} 

\label{fig:fb-HoLE-Conf}
...
\begin{frame}\frametitle{Open Research Questions}
\small{
...
\begin{frame}\frametitle{Commonsense KGs}
\end{frame}
