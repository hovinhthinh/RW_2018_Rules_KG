...
Extending the techniques to more advanced rule forms is a promising future direction. %Addressing the rule learning over weighted data has been studied under probabilistic ILP%, where given a set of probabilistic examples for grounded atoms and a target predicate $H$, the task is to learn rules for predicting probabilities of atoms for $H$
...
(see Figure~\ref{fig:discussion_overview}). These external sources can be of different nature ranging from %either be 
