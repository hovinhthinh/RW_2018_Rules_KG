...
with disjunction (\eg $isMale(X) \vee isFemale(X) \leftarrow isPerson(X)$) or rules with existential quantifiers in the head (\eg $(\exists Y: playsInstrument(X, Y)) \leftarrow isMusician(X))$) is an interesting topic to study. % can encode more interesting patterns
