...
\leanparagraph{Learning rules over probabilistic data}  %Nevertheless, e
The majority of existing rule learning approaches over knowledge graphs model KGs as sets of true triples thus totally ignoring possible inaccuracy of some of the facts that they encode. However, since KGs are normally automatically constructed from noisy textual resources, obviously not all of the extracted facts should have the same weight. Learning rules from such noisy KGs by treating all of the facts in the same way might naturally lead to problematic rules, which when applied could propagate the faulty facts. While modeling the KG as an uncertain data source with attached weights on the facts and learning rules over them studied in the area of probabilistic ILP \cite{probilp} is an important research direction, which so far has been considered only within the problems of extracting rules of restricted form, e.g., without negation, disjunction or existential variables in the head. %  reduce the effect of inaccurate facts in the KG 
Extending the existing techniques to more advanced rule forms and %Addressing the rule learning over weighted data has been studied under probabilistic ILP%, where given a set of probabilistic examples for grounded atoms and a target predicate $H$, the task is to learn rules for predicting probabilities of atoms for $H$
...
which quickly grows to sizes that ILP methods cannot handle. Adapting probabilistic rule learning approaches for real-life KGs has only been studied in limited literature such as~\cite{DeRaedt:2015}, keeping a wide room for improvements.
