...
where $H$ is a standard first-order atom of the form $h(\vec{X})$, $B$ is a conjunction of positive atoms of the form $b_1(\vec{Y_1}),\dotsc,b_k(\vec{Y_k})$, and $\naf\ E$ %with slight abuse of notation, 
 is a conjunction of negated atoms $\naf\, b_{k+1}(\vec{Y_{k+1}}),\dotsc,\naf\, b_n(\vec{Y_{n}})$. Moreover,  $\vec{X},\vec{Y_1},\ldots,\vec{Y_{n}}$ are tuples of either variables or constants whose length corresponds to the arity of the predicates $h,b_1,\ldots,b_n$ respectively.
...
The classical definition of answer sets based on the Gelfond-Lifschitz reduct is given as follows.
 \begin{definition}[Gelfond-Lifschitz Reduct \cite{GL1988}, Answer Set]
...
An alternative definition provided in \cite{FLP2011} relies on a different notion of the reduct, which might be more intuitive.
...
An interpretation $I$ of $P$ is an answer set (or stable model) of $P$ iff $I \in \mi{MM(fP^I)}$, where $fP^I$ is the \emph{Faber-Leone-Pfeifer (FLP) reduct} of  $P$, obtained from $Gr(P)$ by keeping only rules $r$, whose bodies are satisfied by $I$, i.e., $fP^I = \{r \in P \,\mid\, r = H \leftarrow B, I \models B\}.$
\end{definition}
...
\noindent            
The relevant part of ground instantiation $Gr(P)$ of $P$ is obtained by substituting $X,Y,Z$ with $\mi{brad, \,ann}$ and $\mi{berlin}$ respectively. For $I=\{${\small$\mi{isMarriedTo(brad{,}ann){,}}$\\$\mi{livesIn(ann{,}berlin)},\mi{livesIn(brad,berlin)}$}$\}$, both the GL-reduct $P^I$ (resp. the FLP-reduct $fP^I$) of $P$ contain \\\small{$\mi{livesIn(ann,berlin)}\leftarrow \mi{livesIn(brad,berlin),isMarriedTo(brad,ann)}$} \normalsize{and the facts (1), (2). As $I$ is a minimal model of $P^I$ (resp. $fP^I$), it holds that $I$ is an answer set of $P$.}}\qed
\end{example}
\normalsize
...
\item Background knowledge $B$, i.e. an extensive set of relations that can be used to induce the definition of $p$; 
...
Formally, 
\begin{definition}[Inductive Learning from Interpretations]\label{def:learnint}
\begin{itemize}
\item[] \textbf{Given:}
\begin{itemize}
