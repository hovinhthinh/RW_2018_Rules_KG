...
$h(\vec{X}), b_1(\vec{Y_1}),\dotsc, b_n(\vec{Y_n})$ are first-order atoms and right-hand side of the rule is a conjunction of atoms. %, $B$ is a conjunction of positive atoms of the form $b_1(\vec{Y_1}),\dotsc,b_k(\vec{Y_k})$, and $E$ is an atom $b_{k+1}(\vec{Y_{k+1}})$.
...
For simplicity, in this work we use the shortcut $H\leftarrow B, \naf\, E$ to denote rules with a single negated atom, i.e., here $H=h(\vec{X}), B=b_1(\vec{Y_1}), \dotsc, b_k(\vec{Y_k}), E=b_{k+1}(\vec{Y_{k+1}})$. 
Moreover, the left-hand side of a rule $r$ is referred to as its head, denoted by $\mi{head(r)}$, while the right-hand side is its body $\mi{body(r)}$. The positive and negative parts of the body are respectively denoted as $\mi{body^+(r)}$ and $\mi{body^-(r)}$. A rule $r$ is called \emph{positive} or
...
An interpretation $I$ of $P$ is an answer set (or stable model) of $P$ iff $I \in \mi{MM(fP^I)}$, where $fP^I$ is the \emph{Faber-Leone-Pfeifer (FLP) reduct} of  $P$, obtained from $Gr(P)$ by keeping only rules $r$, whose bodies are satisfied by $I$, i.e., \begin{center}$\mi{fP^I = \{r \in P \,\mid\, head(r)\leftarrow body(r), I \models body(r)\}}.$\end{center}
\end{definition}
...
\item Background knowledge $T$, i.e. an extensive set of facts over various relations and possibly rules that can be used to induce the definition of $p$; 
