...
\subsection{Traditional ILP Approach for Rule Learning}
\label{subsec:ilp}
...
Table \ref{tab:ilp} provides classifications of some traditional ILP systems that have been introduced in the literature to solve the above supervised learning task. In particular, we could assign them based on some following characteristics \cite{Boytcheva2007OverviewOI}:
\begin{itemize}
\item \emph{incremental-ness}, demonstrates how the examples are processed to the ILP system. In incremental ILP, examples are provided one by one to the learning system continuously. In contrast, non-incremental ILP systems (or empirical ILP systems) require all examples to be available at the beginning and they cannot be changed to change afterwards.
...
\item \emph{single/multiple target predicate learning}, most popular ILP systems focus on learning a single target predicate. Even though multiple predicate ILP learning algorithms are more powerful, they are harder and less efficient.
\item \emph{theory revision}, denotes the possibility of background theory modification. Most ILP systems allowing theory revision are also incremental and be able to learn multiple predicate. Even though changes in the background $B$ are allowed, they should be minimized.
\end{itemize}
