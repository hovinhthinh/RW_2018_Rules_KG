...
\vspace{-.26cm}
\begin{align*}
conf(r,\cG)= \frac{supp(r,\cG)}{supp(B,\naf\ E,\cG)}
\end{align*}
where $supp(r,\cG)$ and $supp(B,\naf\ E,\cG)$ are the \textit{rule support} and \textit{body support}, respectively, which are defined as follows:
\begin{align*}
supp(r,\cG)= \#(X,Y): H \in \cG, \exists \vec{Z}\;B\in \cG,E \not \in \cG\\
supp(B,\naf\ E,\cG)= \#(X,Y):\exists \vec{Z}\; B\in \cG, E \not \in \cG
\end{align*}
