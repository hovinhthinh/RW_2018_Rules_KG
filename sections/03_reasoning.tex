...
\label{sec:reasoning}
In this section we briefly review concepts from logic programming (see \cite{DBLP:conf/rweb/EiterIK09} for more details) and describe the notion of relational association rules \cite{warmer}.
...
\begin{example}
Consider $\mi{r_2}$ from Section~\ref{sec:intro}. We have that $\mi{head(r)}=\{livesIn(Y,Z)\}$, while $\mi{body^+(r)=\{isMarriedTo(X,Y), livesIn(X,Z)\}}$, and moreover,\\ $\mi{body^-(r)=\{not\;researcher(Y)\}}$. \qed
\end{example}
