...
\subsection{Reasoning over KGs} Rule-based reasoning is used for KGs refinement to solve the inaccuracy and incompleteness~\cite{DBLP:journals/semweb/Paulheim17}. KG completion is one of the main reasoning tasks performed on the KG that is concerned with predicting the missing facts in the KG using the rules and currently existing facts. More formally, 


 \begin{definition}[Rule-based KG completion]\label{def:def:kgcomp} Let $\cG$ be a KG and $\cA$ its factual representation over the signature $\Sigma_{\cG}=\tuple{\mathbf{C},\mathbf{R},\cC}$. Let, moreover, $\cR$ be a set of rules %mined from $\cG$,
 i.e. rules over the signature $\Sigma_{\cR}=\tuple{\mathbf{C}\cup \mathbf{R},\cC}$. % mined from $\cG$.
 Then \emph{completion of $\cG$ (resp. $\cA$) \wrt\ $\cR$} is a graph $\cG_{\cR}$ constructed from any answer set $\cG_{\cR}\in AS(\cR \cup \cA)$. 
 \end{definition}
