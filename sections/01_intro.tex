...
huge graph-structured knowledge bases (KBs) also known as knowledge graphs (KGs) such as NELL \cite{nell}, DBpedia \cite{dbpedia}, YAGO \cite{yago} and Wikidata \cite{wikidata}. These KGs contain millions or billions of relational facts in the form of subject-predicate-object (SPO) triples. 
...
For the KG curaction, this has twofold benefits. First, since KGs operate under the Open World
...
$\mi{r_1}$ mined from the graph in Fig.~\ref{rdf}, the missing living place of Dave can be deduced based on the data about his wife Clara. Second, rules can be used to eliminate erroneous facts in the KG. For example, assuming
...
Horn rules, where all predicates in the rule body are positive are insufficiently expressive % .
% This is insufficient
to capture rules that have exceptions, such as
\emph{``Married people live in the same place unless one is a
  researcher''}:\\ $\mi{r_2{:}livesIn(Y,Z){\leftarrow}
  isMarriedTo(X,Y){,}livesIn(X,Z){,}{\naf\,} researcher(Y)}$. This additional knowledge could be an explanation for Alice living in an unexpected place. If $\mi{r_2}$ often holds, then
one can no longer complete the missing living place for Dave by assuming
that he lives with his wife Clara.  Thus, understanding exceptions is crucial for KG completion and curation \cite{DBLP:conf/semweb/Gad-ElrabSUW16,rumis}.

Moreover, since rules are learned from incomplete KGs, they might be themselves erroneous and might make incorrect predictions on missing facts. 
 E.g., $\mi{r_3}: \mathit{hasChild(X,Y)}\leftarrow \mathit{worksAt}(X,Z),\mathit{educatedAt}(Y,Z)$ could be mined from a highly incomplete and biased KG stating that workers of certain institutions often have children among the people educated there, 
...
This additional knowledge can effectively exploited in the rule learning process as shown in \cite{carl}.
