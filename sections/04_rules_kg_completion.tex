...
%In the rest of 
Inspired by the pac-learning approach \cite{DBLP:journals/cacm/Valiant84} we present the vie w that the world is very complicated and there is no hope of acquiring an exact representation of it; our aim should be to acquire enough information with which to cope effecti vely in the world. In doing so we extract certain regularities from the world, and assume that in similar circumstances we can rely on these. 
