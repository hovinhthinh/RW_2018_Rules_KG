...
The majority of the classical existing rule induction methods mentioned in Section~\ref{sec:reasoning} operate under the closed world assumption and aim at exacting rules that perfectly satisfy the criterias of Definition~\ref{def:learnex} or Definition~\ref{def:learnint}.
On the other hand, the real world is very complicated, and its exact representation often cannot be acquired. Thus, in the context of KGs, one normally aims at extracting certain regularities from the data, might not neccessarily be perfectly correct, but when seen as rules produce many true facts.
