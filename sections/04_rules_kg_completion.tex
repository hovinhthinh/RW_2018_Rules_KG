...
all person's children are present in the KG. This assumption has turned out to be indeed valid in real-world KGs for some topics \cite{amie}. However, its effectiveness is questionable when applied on highly incomplete KGs as experienced in~\cite{thinh2018}.
Formally, 
...
that are true in $\cG^i$, i.e., $UP(r,\cG)=\beta*\#(x,y):h(x,y)\not \in \cG$, and propose several ways for computing $\beta$ both empirically via sampling and theoretically based on properties of $\cG$.

Finally, provided that there is a way to estimate $|UP(r,\cG)|$, we have the following formula for the \emph{rc-confidence}:
\[
