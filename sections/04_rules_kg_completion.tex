...
\[
\mi{conf_{pca}}(r, \cG) := \frac{\mi{supp}(r, \cG)}{\#(X,Y): \exists \vec{Z}: B \in \cG, E \notin \cG  \wedge \exists Y': h(X,Y')\in \cG}
\]
...
\[conf_{st}(r, \cG) = \frac{supp(r, \cG)}{supp(B,\naf\ E,\cG) - \sum_{e \in U}P(e,p,\cG)} \]
...
\begin{align*}
\mi{conf_{comp}}(r,\cG) := \frac{\mi{supp}(r,\cG)}{\mi{supp}(B,\naf\ E,\cG) - \mi{npi}(r,\cG)}
\end{align*}
...
\begin{align*}
\mi{precision_{comp}}(r,\cG)=1-\frac{\mi{npc}(r,\cG)}{\mi{supp}(B,\naf\ E,\cG)}\\
