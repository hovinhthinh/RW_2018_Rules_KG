...
\begin{example}
For instance, consider a KG in Figure~\ref{fig:fam_grad} \cite{carl}, which presents information about scientific families.
The heavily biased rule $\mi{r_1':\;hasChild(X,Y)\leftarrow worksAt(X,Z), educated(Y,Z)}$ mentioned in Section~\ref{sec:intro} can be mined from it along with a reasonable rule $\mi{r_2':\,hasSibling(X,Z)\leftarrow hasFather(X,Y),hasChild(Y,Z)}$, stating that people with the same father are likely siblings. We have that $\mi{conf(\mi{r_1'})}=\frac{2}{8}$, while
 %we have that 
$\mi{conf(\mi{r_2'})}=\frac{1}{6}$, reflecting counterintuitive rule preference. \qed
\end{example}
