...
%then he/she should 
live somehere. This rule cannot infer the exact living place of a person, but merely its existence, and thus is less interesting. \qed% for the KG completion. %location 
...
\[\mi{conf_{st}(r, \cG) = \frac{\textit{r-supp}(r, \cG)}{\textit{b-supp}(r,\cG) - \sum_{e \in U^r}Pr(e,h,\cG)}} \]
where $U^r$ is the set of entities % previously
that have no outgoing %relations of 
$h$-edges in $\cG$, but do have some in $\cG_r$, and $Pr(e,h,\cG)$ is the probability of the entity $e$ having the relation $h$, approximated using entity \textit{type} information \cite{rdf2rules}. More specifically,
\[\mi{Pr(e,h,\cG)  = max_{t \in T_{e}}\frac{|Inst_h(t, \cG)|}{|Inst(t, \cG)|}{}}\]
where $T_{e}=\{t\,|\,t(e)\in \cG\}$, $\mi{Inst(t, \cG)=\{e'\,|\,t(e')\in \cG\}}$, and, moreover, \\$\mi{Inst_h(t, \cG)=\{e'\in Inst(t,\cG)\,|\,h(e',e'')\in\cG\}}$.
Intuitively, soft confidence is designed to avoid the underfitting of standard confidence and overfitting of PCA confidence by accounting for the probability of entities having certain relations.
%having the head predicate in the unknown part of the rule.
\begin{example}
Consider the KG $\cG$ from Figure~\ref{rdf}, we have $U^{r_1} = \{\mi{lucy, dave}\}$, $U^{r_2} = \{\mi{lucy}\}$. Moreover, $\mi{Pr(dave,livesIn,\cG)} = \frac{1}{2}$, since $\mi{dave}$ has only 1 type $\mi{researcher}$, and in total the KG stores 
%there are totally 
...
\subsubsection{RC Confidence} Another rule measure, which similarly to completeness-aware measures attempts to rely on assumptions about the tuples not in the KG when evaluating the quality of a potential rule has been proposed in \cite{DBLP:conf/www/ZupancD18}. 
...
\[\frac{\textit{r-supp}(r,\cG)}{\#(x,y):h(x,y) \in  \cG} = \frac{|UP(h,\cG)|}{\#(x,y):h(x,y) \in \cG^i\backslash \cG}\]
...
Now provided that there is a way to estimate $|UP(r,\cG)|$, we have the following formula for the \emph{rc-confidence}:

\[
conf_{rc}(r,\cG) = \frac{\textit{r-supp}(r,\cG) + |UP(r,G)|}{\textit{b-supp}(r,\cG)}
\]

To approximately determine the size of $UP(r,\cG)$ in \cite{DBLP:conf/www/ZupancD18} the authors rely on the proportion $\beta$ of all the unlabeled examples
that are true in $\cG^i$, i.e., $\beta=\dfrac{|UP(r,\cG)}{|U()|}$, and propose several ways of determining based only on $\cG$.
