...
For the KG in Figure~\ref{rdf} and the earlier introduced rules $r_1$ and $r_2$, it holds that $\mi{conf_{pca}(r_1,\cG) = \frac{3}{4}}$, since we do not know any living places of $\mi{lucy}$ and $\mi{dave}$. Similarly, $\mi{conf_{pca}(r_2,\cG) = \frac{3}{3}}$.
...
\[\mi{conf_{st}(r, \cG) = \frac{\textit{r-supp}(r, \cG)}{\textit{b-supp}(r,\cG) - \sum_{e \in U}Pr(e,h,\cG)}} \]
where $U$ is the set of entities % previously
...
Here, $C_{e}$ contains all types of entity $e$, $Inst(c, \cG)$ stores all entities of the type $t$, and $Inst_h(c, \cG)$ is similar to $Inst(t, \cG)$, but restricts  only to entities that have some outgoing $h$-link.
