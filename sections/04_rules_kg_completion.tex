...
Since the source of the data from which one starts the rule extraction process is a KG, i.e., a set of facts over various relations, the closest to the rule-based KG completion setting is the classical ILP task of learning from interpretations. However, in the latter, the CWA is typically adopted, which makes it not suitable for incomplete KGs.

Reusing the methods that induce logical theories from a set of positive and negative examples and background knowledge is likewise not possible due to the following important obstacles. % that prevent one from employing the off-the-shelf traditional ILP algorithms for solving the above rule-based KG completion problem. 
