...
On the other hand, as discussed in Section~\ref{sec:kgs}, knowledge graphs are generally incomplete, i.e., they might miss some true facts, e.g., living places of scientists. Moreover, the real world is very complicated, and its exact representation often cannot be acquired. Thus, in the context of KGs, one normally aims at extracting certain regularities from the data, which are not universally correct, but when seen as rules predict many true facts. 

Therefore, the goal of automatic rule-based KG completion is to learn a set $P$ of logic rules from the available graph $\cG$, such that their application on $\cG$ results in a good approximation of the ideal graph $\cG^i$. More formally,
