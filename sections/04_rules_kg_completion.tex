...
The majority of the classical existing rule induction methods mentioned in Section~\ref{sec:reasoning} assume that the given data from which the rules are induced is complete, accurate and representative. Therefore, they rely on CWA and aim at extracting rule hypotheses that perfectly satisfy the criteria from Definition~\ref{def:learnex} or Definition~\ref{def:learnint}.
On the other hand, as discussed in Section~\ref{sec:kg_realm}, knowledge graphs are highly incomplete, noisy and biased. Moreover, the real world is very complicated, and its exact representation often cannot be acquired from the data, meaning that the task of inducing a perfect rule set from a KG is typically unfeasible. Therefore, in the context of KGs, one normally aims at extracting certain regularities from the data, which are not universally correct, but when seen as rules predict a sufficient portion of true facts. 

In other words, the goal of automatic rule-based KG completion is to learn a set $R$ of logic rules from the available graph $\cG$, such that their application on $\cG$ results in a good approximation of the ideal graph $\cG^i$. 
More formally,
...
%Formally, the task of rule-based KG completion is defined as follows:
 ...
 \begin{definition}[Rule-based KG Completion]\label{def:kgcomp} Let $\cG$ be a KG  over the signature $\Sigma_{\cG}=\tuple{\cR,\cC}$. Let, moreover, $R$ be a set of rules with predicates from $\cR$ induced from $\cG$.  Then \emph{rule-based completion of $\cG$ \wrt\ $R$} is a graph $\cG_{R}$ constructed from any answer set $\cG_{R}\in AS(R \cup \cG)$. 
 \end{definition}
...
If the ideal graph $\cG^i$ was known, then the problem of rule-based KG completion would be essentially the same as the task of learning from interpretations given in Definition~\ref{def:learnint}, with $\cG$ playing a role of the background knowledge $T$ and $\cG^i$ the interpretation $I$. %The closest to the rule-based KG completion setting is the classical ILP task of learning from interpretations from Definition~\ref{def:learnint}. 
However, unfortunately $\cG^i$ is unknown, and it cannot be easily reconstructed, since KGs follow OWA. % in the task of learning from interpretations, the CWA is typically adopted, which makes it not suitable for incomplete KGs. R

Moreover, reusing the methods that induce logical theories from a set of positive and negative examples from Definition~\ref{def:learnex} is likewise not possible due to the following important obstacles \cite{rumis}. % that prevent one from employing the off-the-shelf traditional ILP algorithms for solving the above rule-based KG completion problem. 
...
steps: rule construction and rule assessment. % While the main concern of the rule construction phase is how to mine frequent patterns from the KG, 
...
\begin{center}$\mi{Q(X,Y,Z)\text{ :- }marriedTo(X,Y),\, }\mi{livesIn(X,Z)}$
\end{center}
...
\begin{center}$Q'(X,Y,Z)\text{ :- }\mi{marriedTo(X,Y),\,livesIn(X,Z),\,} \mi{livesIn(Y,Z)}$
\end{center} we can construct the association rule $Q \Rightarrow Q'$. \qed
 \end{example}
...
For $r:\;\mi{H\leftarrow B, \naf\ E}$ (see Equation~\ref{eq:ruleshort}), with $H=\mi{h(X,Y)}$, $B,E$ involving variables from $\vec{Z}\supseteq \{X,Y\}$ and a KG $\cG$, the \emph{(standard) confidence} is given as:
\begin{align*}
...
\end{align*}
\begin{example}
For the KG in Figure~\ref{rdf}, we have $\textit{rel-supp}(livesIn, \cG) = \frac{10}{10 \times 4} = \frac{1}{4}$, thus $conv(r_1,\cG) = \frac{1-\frac{1}{4}}{1-\frac{3}{6}} = \frac{3}{2}$ and $conv(r_2,\cG) = \frac{1-\frac{1}{4}}{1-\frac{3}{4}} = 3$.\qed
\end{example}
...
The rules $r_1$ and $r_2$ from Section~\ref{sec:intro} are \textit{closed}. % An example of a \textit{non-closed} rule that can be learned from the KG in Figure~\ref{rdf} is:
...
%then he/she should 
live somewhere. This rule cannot infer the exact living place of a person, but merely its existence, and thus it is less interesting in this context. \qed% for the KG completion. %location 
...
Apart from a KG, %it expects from the user a support threshold and a maximum rule length.
it expects the maximum length of the rule and the threshold value for its support.   
In AMIE, rules are treated as sequences of atoms, where the first atom is the head, and subsequent atoms form the body of the rule. The algorithm maintains a queue of intermediate rules, which initially stores a single rule with an empty body for every KG relation. 
...
While RDF2Rules performs the rule extraction faster than AMIE due to an effective pruning strategy
...
The ranking metrics from data mining such as support and confidence (see e.g., \cite{metrics-summary} for overview of others) presented in Section~\ref{subsec:assoc_rule} have been designed for datasets treated under the CWA, and they can be counterintuitive for the KGs, in which facts are largely missing.

\begin{example}\label{ex:rulesforgprime}
For instance, consider the KG $\cG'$ in Figure~\ref{fig:fam_grad} \cite{carl}, which presents information about scientific families. 
...
whenever at least one object for a given subject and a predicate is in the KG (e.g., ``Eduard
...
\]
However, the effectiveness of the PCA confidence decreases when applied on highly incomplete KGs as experienced in~\cite{thinh2018}.
\begin{example}
...
\subsubsection{(In)completeness Metadata.}
...
\end{equation}
These cardinality statements can be obtained %using
via Web extraction techniques~\cite{cardinality-extraction-iswc-2016}.
...
Unlike other state-of-the-art rule mining systems, the %recent
 algorithm of %the focus of a recent % the novel 
 \emph{ontological pathfinding} (OP) \cite{op} focuses on %algorithm is % not at how to find the candidate rules in the knowledge graph, but rather at
% how to 
