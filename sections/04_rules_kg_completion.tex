...
If the ideal graph $\cG^i$ was known, then the problem of rule-based KG completion would be essentially the same as the task of learning from interpretations given in Definition~\ref{def:learnint}, with $\cG$ playing a role of the background knowledge and $\cG^i$ the interpretation $I$. %The closest to the rule-based KG completion setting is the classical ILP task of learning from interpretations from Definition~\ref{def:learnint}. 
However, unfortunately $\cG^i$ is unknown, and it cannot be reconstructed as KGs follow OWA. % in the task of learning from interpretations, the CWA is typically adopted, which makes it not suitable for incomplete KGs. R
