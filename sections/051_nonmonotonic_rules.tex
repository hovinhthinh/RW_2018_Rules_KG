...
$\mi{r_3:  isPoliticianOf(X, Z)}  \leftarrow
 ...
 \vspace{-.6cm}
\end{figure}
\leanparagraph{Example Rules} Figure~\ref{fig:examplerules} shows examples of revised rules obtained in \cite{rumis} by the described method. For example, $\mi{r_1}$ extracted from IMDB states that movie plot writers stay the same throughout the sequel unless a movie is American, while $\mi{r_2}$ reflects that spouses of movie directors often appear on the cast with the exception of actors from of old silent movies. Finally, the rule $\mi{r_3}$ learned from YAGO says that ancestors of politicians are also politicians in the same country with the exception of Mexican vice-presidents.
...
\begin{figure}[t]
    ...
    \begin{tabular}{l}
 {\scriptsize
        $\mi{r_1: nationality(X, Y)}  \leftarrow
        \mi{graduatedFrom(X, Z)},\mi{inCountry(Z,Y)},$ $ \textbf{not}$  $\mi{researchUni(Z)} $}\\        
          {\scriptsize 
$\mi{r_2:  nobleFamily(X, Y)}  \leftarrow
 ...
 \vspace{-.6cm}
\end{figure}
\leanparagraph{Example Rules} Figure~\ref{fig:examplerules} presents examples of rules learned by the RuLES system from the Wikidata dataset. The first rule $\mi{r_1}$ says that a person is a citizen of the country where his alma mater is located, unless it is a research institution, since most researchers in
