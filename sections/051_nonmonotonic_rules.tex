...
association rule measure \cite{Azevedo2007}. \gad{text updated here!} Conversely, $q_{\mi{conflict}}$ estimates the number of conflicting predictions that the rules in $R$ generate. To measure $\mi{q_{conflict}}$ for $R$,
 an extended set of rules $R^{aux}$ is created to materialize the predictions prevented by the negative part of each individual rule in $R$%, which
. $R^{aux}$ contains every %every
