...
Combinations of exception candidates could be an explanation for some missing links, so the search space of solutions to the above problem is large. In \cite{rumis} a restriction to a single predicate as a final exception has been posed; %extensions
...
$\mi{r^2:  nobleFamily(X, Y)}  \leftarrow
 ...
 \vspace{-.6cm}
\end{figure}
\leanparagraph{Example Rules} Figure~\ref{fig:examplerules} presents examples of rules learned by the RuLES system from the Wikidata KG. The first rule $\mi{r_1}$ says that a person is a citizen of the country where his alma mater is located, unless it is a research institution, since most researchers in
